...
def create_img(img_path, img_res_path, model_path, target_str, target_idx, des_conf=0.95):
    ...
    while solved == False:
        ...
        if(get_predictions(test, model)[0][1] == TREE_FROG_STR and get_predictions(test, model)[0][2] > 0.95):
            ...
            image = ndimage.imread(FILENAME+'_mod.png')
            #Add noise generation algorithm here
            #Add code for image augmentation here (add the noise to the image)
            ...
            #Add code here to check if the augmented image has an improved confidence, if it does, save a copy.
